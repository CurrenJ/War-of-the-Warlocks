...
public class ItemGraphics{
	...
	public void drawItem(Item item, Graphics g, Camera camera) throws Exception{
		Image itemImage = item.getImage().getScaledInstance(item.getWidth(), item.getHeight(), 0);
		int relativeX = item.getX() - camera.getCamX();
		int relativeY = (item.getY() - item.getHeight()) - camera.getCamY();
