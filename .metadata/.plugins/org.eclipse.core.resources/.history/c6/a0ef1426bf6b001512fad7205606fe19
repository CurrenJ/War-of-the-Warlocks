...
public class BackgroundGraphics {
	...
	public void drawBackground(ParallaxBackgroundSet background, Graphics2D g, Camera camera){
		int xOffset = camera.getCamX() % (BackgroundGraphics.backgroundImages.get(background.getPrefix() + "_0").getWidth(panel));
		for(int l = background.getLayers(); l > 0; l--)
			for(int d = 0; d < background.getDupesPossibleOnScreen()+1; d++)
				g.drawImage((Image) BackgroundGraphics.backgroundImages.get(background.getPrefix() + "_" + (l-1)), ((-1 * xOffset / (l)) + (BackgroundGraphics.backgroundImages.get(background.getPrefix() + "_" + (l-1)).getWidth(panel) * d)) - (BackgroundGraphics.backgroundImages.get(background.getPrefix() + "_0").getWidth(panel)), 0, panel);
